<div class="front_page_section front_page_section_features<?php
	$ozeum_scheme = ozeum_get_theme_option( 'front_page_features_scheme' );
	if ( ! empty( $ozeum_scheme ) && ! ozeum_is_inherit( $ozeum_scheme ) ) {
		echo ' scheme_' . esc_attr( $ozeum_scheme );
	}
	echo ' front_page_section_paddings_' . esc_attr( ozeum_get_theme_option( 'front_page_features_paddings' ) );
	if ( ozeum_get_theme_option( 'front_page_features_stack' ) ) {
		echo ' sc_stack_section_on';
	}
?>"
		<?php
		$ozeum_css      = '';
		$ozeum_bg_image = ozeum_get_theme_option( 'front_page_features_bg_image' );
...
<?php
	// Add anchor
	$ozeum_anchor_icon = ozeum_get_theme_option( 'front_page_features_anchor_icon' );
	$ozeum_anchor_text = ozeum_get_theme_option( 'front_page_features_anchor_text' );
...
?>
	...
	<?php
	if ( ozeum_get_theme_option( 'front_page_features_fullheight' ) ) {
	...
	?>
			...
			<?php
			$ozeum_css      = '';
			$ozeum_bg_mask  = ozeum_get_theme_option( 'front_page_features_bg_mask' );
			$ozeum_bg_color_type = ozeum_get_theme_option( 'front_page_features_bg_color_type' );
			if ( 'custom' == $ozeum_bg_color_type ) {
				$ozeum_bg_color = ozeum_get_theme_option( 'front_page_features_bg_color' );
	...
	>
		<div class="front_page_section_content_wrap front_page_section_features_content_wrap content_wrap">
			<?php
			// Caption
			$ozeum_caption = ozeum_get_theme_option( 'front_page_features_caption' );
			if ( ! empty( $ozeum_caption ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<h2 class="front_page_section_caption front_page_section_features_caption front_page_block_<?php echo ! empty( $ozeum_caption ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( $ozeum_caption, 'ozeum_kses_content' ); ?></h2>
				<?php
			}

			// Description (text)
			$ozeum_description = ozeum_get_theme_option( 'front_page_features_description' );
			if ( ! empty( $ozeum_description ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<div class="front_page_section_description front_page_section_features_description front_page_block_<?php echo ! empty( $ozeum_description ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( wpautop( $ozeum_description ), 'ozeum_kses_content' ); ?></div>
				<?php
			}
			...
			if ( is_active_sidebar( 'front_page_features_widgets' ) ) {
				...
				if ( ! ozeum_exists_trx_addons() ) {
					...
					ozeum_customizer_need_widgets_message( 'front_page_features_caption', 'ThemeREX Addons - Services' );
				}
			}
			?>
			</div>
