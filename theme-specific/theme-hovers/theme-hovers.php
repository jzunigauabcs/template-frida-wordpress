...
if ( ! function_exists( 'ozeum_hovers_theme_setup3' ) ) {
	add_action( 'after_setup_theme', 'ozeum_hovers_theme_setup3', 3 );
	function ozeum_hovers_theme_setup3() {

		// Add 'Buttons hover' option
		ozeum_storage_set_array_after(
			'options', 'border_radius', array(
				'button_hover' => array(
					'title'   => esc_html__( "Button's hover", 'ozeum' ),
					'desc'    => wp_kses_data( __( 'Select hover effect to decorate all theme buttons', 'ozeum' ) ),
					'std'     => 'default',
					'options' => array(
						'default'      => esc_html__( 'Fade', 'ozeum' ),
					),
					'type'    => 'select',
				),
				'image_hover'  => array(
					'title'    => esc_html__( "Image's hover", 'ozeum' ),
					'desc'     => wp_kses_data( __( 'Select hover effect to decorate all theme images', 'ozeum' ) ),
					'std'      => 'view',
					'override' => array(
						'mode'    => 'page',
						'section' => esc_html__( 'Content', 'ozeum' ),
					),
					'options'  => ozeum_get_list_hovers(),
					'type'     => 'select',
				),
			)
		);
...
if ( ! function_exists( 'ozeum_hovers_theme_setup9' ) ) {
	add_action( 'after_setup_theme', 'ozeum_hovers_theme_setup9', 9 );
	function ozeum_hovers_theme_setup9() {
		add_action( 'wp_enqueue_scripts', 'ozeum_hovers_frontend_scripts', 1010 );
		add_action( 'wp_enqueue_scripts', 'ozeum_hovers_frontend_styles', 1100 );
		add_action( 'wp_enqueue_scripts', 'ozeum_hovers_responsive_styles', 2000 );
		add_filter( 'ozeum_filter_localize_script', 'ozeum_hovers_localize_script' );
		add_filter( 'ozeum_filter_merge_scripts', 'ozeum_hovers_merge_scripts' );
		add_filter( 'ozeum_filter_merge_styles', 'ozeum_hovers_merge_styles' );
		add_filter( 'ozeum_filter_merge_styles_responsive', 'ozeum_hovers_merge_styles_responsive' );
		add_filter( 'ozeum_filter_get_css', 'ozeum_hovers_get_css', 10, 2 );
	}
}

// Enqueue hover styles and scripts
if ( ! function_exists( 'ozeum_hovers_frontend_scripts' ) ) {
	
	function ozeum_hovers_frontend_scripts() {
		if ( ozeum_is_on( ozeum_get_theme_option( 'debug_mode' ) ) ) {
			$ozeum_url = ozeum_get_file_url( 'theme-specific/theme-hovers/theme-hovers.js' );
			if ( '' != $ozeum_url ) {
				wp_enqueue_script( 'ozeum-hovers', $ozeum_url, array( 'jquery' ), null, true );
			}
		}
	}
}
...
if ( ! function_exists( 'ozeum_hovers_frontend_styles' ) ) {
	
	function ozeum_hovers_frontend_styles() {
		if ( ozeum_is_on( ozeum_get_theme_option( 'debug_mode' ) ) ) {
			$ozeum_url = ozeum_get_file_url( 'theme-specific/theme-hovers/theme-hovers.css' );
			if ( '' != $ozeum_url ) {
				wp_enqueue_style( 'ozeum-hovers', $ozeum_url, array(), null );
			}
		}
	}
}
...
if ( ! function_exists( 'ozeum_hovers_responsive_styles' ) ) {
	
	function ozeum_hovers_responsive_styles() {
		if ( ozeum_is_on( ozeum_get_theme_option( 'debug_mode' ) ) ) {
			$ozeum_url = ozeum_get_file_url( 'theme-specific/theme-hovers/theme-hovers-responsive.css' );
			if ( '' != $ozeum_url ) {
				wp_enqueue_style( 'ozeum-hovers-responsive', $ozeum_url, array(), null );
			}
		}
	}
}
...
if ( ! function_exists( 'ozeum_hovers_merge_styles' ) ) {
	
	function ozeum_hovers_merge_styles( $list ) {
...
if ( ! function_exists( 'ozeum_hovers_merge_styles_responsive' ) ) {
	
	function ozeum_hovers_merge_styles_responsive( $list ) {
...
if ( ! function_exists( 'ozeum_hovers_localize_script' ) ) {
	
	function ozeum_hovers_localize_script( $arr ) {
		$arr['button_hover'] = ozeum_get_theme_option( 'button_hover' );
		return $arr;
	}
}

// Merge hover effects to the single js
if ( ! function_exists( 'ozeum_hovers_merge_scripts' ) ) {
	
	function ozeum_hovers_merge_scripts( $list ) {
...
if ( ! function_exists( 'ozeum_hovers_add_icons' ) ) {
	function ozeum_hovers_add_icons( $hover, $args = array() ) {
		...
		if ( in_array( $hover, array( 'icons', 'zoom' ) ) ) {
			...
			?>
			<div class="icons">
				<a href="<?php echo ! empty( $post_link ) ? esc_url( $post_link ) : esc_attr( $no_link ); ?>" <?php ozeum_show_layout($target); ?> aria-hidden="true" class="icon-link
									<?php
			...
			<div class="icons">
						...
						apply_filters(
							...
							'<a rel="nofollow" href="' . esc_url( $product->add_to_cart_url() ) . '" 
														...
														class="shop_cart icon-cart-2 button add_to_cart_button'
																...
																. '">'
											. ( 'shop_buttons' == $hover ? ( $product->is_type( 'variable' ) ? esc_html__( 'Select options', 'ozeum' ) : esc_html__( 'Buy now', 'ozeum' ) ) : '' )
										. '</a>',
							$product
						)
					);
				...
				if ( 'shop_buttons' == $hover ) {
					if ( is_object( $args['cat'] ) ) {
						esc_html_e( 'View products', 'ozeum' );
					} else {
						esc_html_e( 'Details', 'ozeum' );
					}
				}
				?>
				</a>
                        ...
                        apply_filters(
                            ...
                            '<a rel="nofollow" href="' . esc_url( $product->add_to_cart_url() ) . '" 
														...
														class="shop_cart button add_to_cart_button'
                                                            ...
                                                            . ( $product->supports( 'ajax_add_to_cart' ) ? ' ajax_add_to_cart' : '' )
                                                            . '">'
                            . ( 'simple' == $hover ? ( $product->is_type( 'variable' ) ? esc_html__( 'Buy Now', 'ozeum' ) : esc_html__( 'Add to Cart', 'ozeum' ) ) : '' )
                            . '</a>',
                            $product
                        )
                    );
            ...
            // Hover style 'Icon'
            ?>
            <div class="icons"><a href="<?php echo ! empty( $post_link ) ? esc_url( $post_link ) : $no_link; ?>" <?php ozeum_show_layout($target); ?> aria-hidden="true" class="icon-search-alt"></a></div>
            <?php

        } elseif ( 'dots' == $hover ) {
            // Hover style 'Dots'
            ?>
            <a href="<?php echo ! empty( $post_link ) ? esc_url( $post_link ) : $no_link; ?>" <?php ozeum_show_layout($target); ?> aria-hidden="true" class="icons"><span></span><span></span><span></span></a>
            <?php

        } elseif ( 'view' == $hover ) {
            // Hover style 'Dots'
            ?>
            <a href="<?php echo ! empty( $post_link ) ? esc_url( $post_link ) : $no_link; ?>" <?php ozeum_show_layout($target); ?> aria-hidden="true" class="view"><span><?php esc_html_e( 'View', 'ozeum' )?></span></a>
            <?php
            ...
            <div class="post_info">
                <div class="post_info_back">
                    <h4 class="post_title">
                        ...
                        if ( ! empty( $post_link ) ) {
                        ?>
                        <a href="<?php echo esc_url( $post_link ); ?>" <?php ozeum_show_layout($target); ?>>
                            <?php
                            }
                            the_title();
                            if ( ! empty( $post_link ) ) {
                    ...
                    <div class="post_descr">
                        ...
                        if ( 'excerpt' == $hover ) {
                            ...
                            <div class="post_excerpt"><?php
                                ozeum_show_layout( get_the_excerpt() );
                                ?></div>
                            <?php
                        }
                        ?>
            ...
            // Hover style empty
            ?>
            <a href="<?php echo esc_url( $post_link ); ?>" <?php ozeum_show_layout($target); ?> aria-hidden="true" class="icons"></a>
            <?php
        }
    }
}
...
// Add styles into CSS
if ( ! function_exists( 'ozeum_hovers_get_css' ) ) {
    
    function ozeum_hovers_get_css( $css, $args ) {
...
.post_featured.hover_view .view span:hover {
...
.post_featured.hover_simple .simple a:hover {
    color: {$colors['text_link']};
    background-color: transparent;
}
