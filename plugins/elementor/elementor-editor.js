...
jQuery( document ).ready(
	function() {
		...
		ozeum_add_filter('trx_addons_filter_sc_classes', function(classes, settings) {
		...
		if (window.elementor !== undefined) {
			...
			jQuery( '#elementor-panel' )
				.on( 'input change', '[data-setting*="ozeum_options_"]', function (e) {
					...
					var link = jQuery( this ).parents( '.elementor-control' ).find( 'a.ozeum_post_editor' );
					if ( link.length > 0 ) {
						ozeum_change_post_edit_link_elementor( link );
					}
				} )
				.on( 'click', '.ozeum_post_editor', function(e) {
					ozeum_change_post_edit_link_elementor( jQuery(this) );
					if (jQuery(this).hasClass('ozeum_hidden' )) {
						e.preventDefault();
						return false;
					}
				});
		...
		function ozeum_change_post_edit_link_elementor(a) {
			if (a.length > 0) {
